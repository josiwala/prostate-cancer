% references
%
% REFERENCES: A list of the references used in the article.
%
%----------------------------------------------------------------------------------------
%	PACKAGES AND OTHER DOCUMENT CONFIGURATIONS
%----------------------------------------------------------------------------------------
%

\section{References}



%\noindent - Kutner, M. H., Nachtsheim, C. J., \& Neter, J. (2004). \textit{Applied Linear Regression Models} (4\textsuperscript{th} ed.). N.p.: McGraw-Hill/Irwin. \\

%\noindent - Devore, J. (2012). \textit{Probability and Statistics for Engineering and the Sciences} (8\textsuperscript{th} ed.). N.p.: Brooks/Cole. \\

%\noindent - Prostate Cancer: Prostate Cancer Information and Overview. (n.d.). Retrieved November 23, 2020, from https://www.cancer.org/cancer/prostate-cancer.html

\hangindent=\parindent
\hangafter=1
\noindent
- Devore, J. (2012). \textit{Probability and Statistics for Engineering and the Sciences} (8\textsuperscript{th} ed.). N.p.: Brooks/Cole. \\

\hangindent=\parindent
\hangafter=1
\noindent
- DOI: 10.1200/JCO.2019.37.7\_suppl.116 Journal of Clinical Oncology 37, no. 7\_suppl (March 01, 2019) 116-116. \\

\hangindent=\parindent
\hangafter=1
\noindent
- Kutner, M. H., Nachtsheim, C. J., \& Neter, J. (2004). \textit{Applied Linear Regression Models} (4\textsuperscript{th} ed.). N.p.: McGraw-Hill/Irwin. \\

\hangindent=\parindent
\hangafter=1
\noindent
- Prostate Cancer: Prostate Cancer Information and Overview. (n.d.). Retrieved November 23, 2020, from https://www.cancer.org/cancer/prostate-cancer.html \\

